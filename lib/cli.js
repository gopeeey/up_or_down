...
cli.responders.listLogs = () => {
    _logs.list(true, (err, logFileNames) => {
        if (err || !logFileNames) return console.log("Unable to get log files", err);
        if (!logFileNames.length) return console.log("No log files found");

        cli.verticalSpace();

        for (const logFileName of logFileNames) {
            if (!logFileName.includes("-")) continue;
